...
const supabase = createClient(
  "https://addghapkcuhowzxkmtdh.supabase.co",
...
async function startEngine() {
  ...
  engine = instance;
  create_account_func = engine.cwrap("create_account", "number", [
    "number",
    "string",
    "number",
  ]);
...
function mainMenu() {
  ...
  rl.question("Action: ", async (choice) => {
    if (choice === "1") {
      console.log("Total Bank Ledger:", get_total_money_func());
      mainMenu();
    } else if (choice === "2" && isAdmin) {
      await adminOnboardMember();
    } else process.exit(0);
  });
}
